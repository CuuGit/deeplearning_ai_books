...
然后是==反向步骤或者说反向传播步骤==，同样也是第$l$层的计算，==你会需要实现一个函数输入为$da^{[l]}$，输出$da^{[l-1]}$的函数==。一个小细节需要注意，输入在这里其实是$da^{[l]}$以及所缓存的$z^{[l]}$值，之前计算好的$z^{[l]}$值，除了输出$da^{[l-1]}$的值以外，也需要输出你需要的梯度$dW^{[l]}$和$db^{[l]}$，这是为了实现梯度下降学习。
...
![](C:/MyGit/deeplearning_ai_books/images/7cfc4b5fe94dcd9fe7130ac52701fed5.png)
...
![](C:/MyGit/deeplearning_ai_books/images/c13d2a8fa258125a5398030c97101ee1.png)
...
![](C:/MyGit/deeplearning_ai_books/images/53a5b4c71c0facfc8145af3b534f8583.png)
